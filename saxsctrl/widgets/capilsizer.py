...
class CapilSizer(ToolDialog):
    ...
    def __init__(self, credo, title='Find position and thickness of capillaries'):
        ToolDialog.__init__(
            self, credo, title, buttons=('Close', Gtk.ResponseType.CLOSE))
        self._poi = {}
        ...
        self._sampleselector = SampleSelector(
            self.credo, False, shortnames=True)
        ...
        l.set_valign(Gtk.Align.CENTER)
        tab.attach(l, 0, 1, row, row + 1, Gtk.AttachOptions.FILL)
        self._scannumber_entry = Gtk.SpinButton(
            adjustment=Gtk.Adjustment(1, 0, 1e6, 1, 10), digits=0)
        tab.attach(self._scannumber_entry, 1, 2, row, row + 1)
        self._scannumber_entry.connect(
            'value-changed', self._on_scannumber_changed)
        row += 1
        ...
        row += 1
        self._plotderivative_radio = Gtk.RadioButton.new_with_label_from_widget(
            self._plotnormal_radio, 'Plot derivative')
        tab.attach(self._plotderivative_radio, 0, 2, row, row + 1)
        row += 1
        self._plotnormal_radio.set_active(True)

        self._plotnormal_radio.connect(
            'toggled', lambda rb: self._on_signalname_changed(self._signalname_combo))
        ...
        l.set_valign(Gtk.Align.CENTER)
        tab.attach(l, 0, 1, row, row + 1, Gtk.AttachOptions.FILL)
        self._leftpoi_entry = Gtk.SpinButton(
            adjustment=Gtk.Adjustment(0, -1e6, 1e6, 1, 10), digits=4)
        tab.attach(self._leftpoi_entry, 1, 2, row, row + 1)
        self._leftpoi_entry.connect(
            'value-changed', lambda sb: self.recalculate())
        b = Gtk.Button(label='Fit')
        tab.attach(b, 2, 3, row, row + 1)
        b.connect('clicked', lambda button, targetentry, name: self._fitpeak(
            targetentry, name), self._leftpoi_entry, 'left')
        row += 1
        ...
        l.set_valign(Gtk.Align.CENTER)
        tab.attach(l, 0, 1, row, row + 1, Gtk.AttachOptions.FILL)
        self._rightpoi_entry = Gtk.SpinButton(
            adjustment=Gtk.Adjustment(0, -1e6, 1e6, 1, 10), digits=4)
        tab.attach(self._rightpoi_entry, 1, 2, row, row + 1)
        self._rightpoi_entry.connect(
            'value-changed', lambda sb: self.recalculate())
        b = Gtk.Button(label='Fit')
        tab.attach(b, 2, 3, row, row + 1)
        b.connect('clicked', lambda button, targetentry, name: self._fitpeak(
            targetentry, name), self._rightpoi_entry, 'right')
        row += 1
    ...
    def _on_scannumber_changed(self, spinbutton):
        try:
            self._scan = self.credo.subsystems[
                'Files'].scanfile.get_scan(spinbutton.get_value_as_int())
        except ValueError as exc:
            md = Gtk.MessageDialog(self.get_toplevel(), Gtk.DialogFlags.DESTROY_WITH_PARENT | Gtk.DialogFlags.MODAL,
                                   Gtk.MessageType.ERROR, Gtk.ButtonsType.OK, 'No such scan: %d' % spinbutton.get_value_as_int())
    ...
    def _on_signalname_changed(self, combobox):
        ...
        if self._plotnormal_radio.get_active():
            ...
            self._y = np.diff(y)
            ax.set_ylabel(
                'Derivative of ' + self._signalname_combo.get_active_text())
        ax.plot(self._x, self._y, '.-')
        ax.set_xlabel(self._scan.columns()[0])
        ax.set_title(self._scan.comment)
    ...
    def recalculate(self):
        if not (('left' in self._poi) and ('right' in self._poi)):
            return True
        self._thickness_label.set_label(
            str(0.1 * (self._poi['right'] - self._poi['left']).abs()))
        self._position_label.set_label(
            str(0.5 * (self._poi['left'] + self._poi['right'])))
        return True

    def save_thickness(self):
        self._sampleselector.get_sample().thickness = 0.1 * \
            (self._poi['right'] - self._poi['left']).abs()
        ...
        if not self.credo.offline:
            md.format_secondary_text(
                'Sample information was also saved to %s' % self.credo.subsystems['Samples'].configfile)
        else:
            md.format_secondary_markup(
                'However, this information <b>WAS NOT SAVED</b> to %s, since we are in offline mode.' % self.credo.subsystems['Samples'].configfile)
        md.run()
        md.destroy()
        del md
    ...
    def save_position(self):
        if self._scan.columns()[0] == self.credo.subsystems['Samples'].motor_samplex:
            ...
            md.format_secondary_text(
                'Which sample position did we determine just now?')
            md.add_button('X', 1)
            md.add_button('Y', 2)
        ...
        if x_or_y is None:
            ...
            self._sampleselector.get_sample().positionx = 0.5 * \
                (self._poi['left'] + self._poi['right'])
            mesg = 'X position'
        else:
            self._sampleselector.get_sample().positiony = 0.5 * \
                (self._poi['left'] + self._poi['right'])
            mesg = 'Y position'
        self.credo.subsystems['Samples'].save()
        md = Gtk.MessageDialog(self.get_toplevel(), Gtk.DialogFlags.DESTROY_WITH_PARENT | Gtk.DialogFlags.MODAL,
    ...
    def _fitpeak(self, targetentry, name):
        ...
        idx = (self._x >= limits[0]) & (self._x <= limits[1]) & (
            self._y >= limits[2]) & (self._y <= limits[3])
        self._poi[name], hwhm, baseline, amplitude = sastool.findpeak_single(
            self._x[idx], self._y[idx], curve=self._peakfunction_combo.get_active_text())
        targetentry.set_value(self._poi[name])
        xfit = np.linspace(
            self._x[idx].min(), self._x[idx].max(), 10 * len(self._x[idx]))
        if self._peakfunction_combo.get_active_text().upper().startswith('GAUSS'):
            yfit = amplitude * \
                np.exp(-0.5 *
                       (xfit - self._poi[name]) ** 2 / hwhm ** 2) + baseline
        else:
            yfit = amplitude * hwhm ** 2 / \
                (hwhm ** 2 + (self._poi[name] - xfit) ** 2) + baseline
        self.fig.gca().plot(xfit, yfit, 'r-')
        self.fig.gca().text(float(self._poi[name]), float(
            amplitude + baseline), 'Peak at ' + str(self._poi[name]), ha='left', va='bottom')
        self.figcanvas.draw()
        self.recalculate()
