...
class LogDisplay(Gtk.VBox):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.textbuffer.create_mark(
            'log_end', self.textbuffer.get_end_iter(), False)
    ...
    def addlogline(self, message, record):
        ...
        self.textview.scroll_to_mark(
            self.textbuffer.get_mark('log_end'), 0.1, False, 0, 0)
        if record.levelno >= logging.INFO:
            self.label.set_label(record.message.split('\n')[0])
        if record.levelno >= logging.CRITICAL:
            self.image.set_from_icon_name(
                'face-crying', Gtk.IconSize.SMALL_TOOLBAR)
        elif record.levelno >= logging.ERROR:
            self.image.set_from_icon_name(
                'dialog-error', Gtk.IconSize.SMALL_TOOLBAR)
        elif record.levelno >= logging.WARNING:
            self.image.set_from_icon_name(
                'dialog-warning', Gtk.IconSize.SMALL_TOOLBAR)
        elif record.levelno >= logging.INFO:
            self.image.set_from_icon_name(
                'dialog-info', Gtk.IconSize.SMALL_TOOLBAR)
        else:
