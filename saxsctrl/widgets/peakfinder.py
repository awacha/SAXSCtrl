...
class PeakFinder(gtk.Dialog):
    lastfoundpeak = None
    def __init__(self, curve, title='Find peak by fitting...', parent=None, flags=gtk.DIALOG_DESTROY_WITH_PARENT, buttons=(gtk.STOCK_CLOSE, gtk.RESPONSE_CLOSE, gtk.STOCK_ADD, gtk.RESPONSE_APPLY)):
        ...
        vb.pack_start(hbox)
        vb1 = gtk.VBox()
        hbox.pack_start(vb1, False)
        
        vbox_fig = gtk.VBox()
        hbox.pack_start(vbox_fig)
        ...
        f = gtk.Frame('Plotting')
        vb1.pack_start(f, False)
        hb = gtk.HBox()
        f.add(hb)
        l = gtk.Label('Plot type:'); l.set_alignment(0, 0.5)
        hb.pack_start(l, False)
        self.plottype_combo = gtk.combo_box_new_text()
        hb.pack_start(self.plottype_combo)
        ...
        l = gtk.Label('Peak type:'); l.set_alignment(0, 0.5)
        tab.attach(l, 0, 1, row, row + 1, gtk.FILL, gtk.FILL)
        self.peaktype_combo = gtk.combo_box_new_text()
        ...
        tab.attach(self.ampl_cb, 0, 1, row, row + 1, gtk.FILL, gtk.FILL)
        ...
        tab.attach(self.position_cb, 0, 1, row, row + 1, gtk.FILL, gtk.FILL)
        ...
        tab.attach(self.sigma_cb, 0, 1, row, row + 1, gtk.FILL, gtk.FILL)
        ...
        tab.attach(self.offset_cb, 0, 1, row, row + 1, gtk.FILL, gtk.FILL)
        ...
        l = gtk.Label('Periodicity:'); l.set_alignment(0, 0.5)
        tab.attach(l, 0, 1, row, row + 1, gtk.FILL, 0)
        self.d_entry = gtk.SpinButton(gtk.Adjustment(58.402, 0, 1e5, 0.1, 1), digits=4)
        tab.attach(self.d_entry, 1, 2, row, row + 1, gtk.FILL | gtk.EXPAND, 0)
        row += 1
        
        l = gtk.Label('Order:'); l.set_alignment(0, 0.5)
        tab.attach(l, 0, 1, row, row + 1, gtk.FILL, 0)
        self.n_entry = gtk.SpinButton(gtk.Adjustment(1, 0, 100, 1, 10), digits=0)
        tab.attach(self.n_entry, 1, 2, row, row + 1, gtk.FILL | gtk.EXPAND, 0)
        row += 1
        ...
        tab = gtk.Table()
        f.add(tab)
        l = gtk.Label('Value'); l.set_alignment(0, 0.5)
        tab.attach(l, 1, 2, 0, 1, gtk.FILL, gtk.FILL)
        l = gtk.Label('Error'); l.set_alignment(0, 0.5)
        tab.attach(l, 2, 3, 0, 1, gtk.FILL, gtk.FILL)
        l = gtk.Label('Amplitude:'); l.set_alignment(0, 0.5)
        tab.attach(l, 0, 1, 1, 2, gtk.FILL, gtk.FILL)
        l = gtk.Label('Position:'); l.set_alignment(0, 0.5)
        tab.attach(l, 0, 1, 2, 3, gtk.FILL, gtk.FILL)
        l = gtk.Label('Sigma:'); l.set_alignment(0, 0.5)
        tab.attach(l, 0, 1, 3, 4, gtk.FILL, gtk.FILL)
        l = gtk.Label('Vert. offset:'); l.set_alignment(0, 0.5)
        tab.attach(l, 0, 1, 4, 5, gtk.FILL, gtk.FILL)
