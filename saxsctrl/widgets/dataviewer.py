...
_errorflags = [('Wrong distance', 'BADDIST'), ('Wrong sample',
...
class DataViewer(ToolDialog):
    ...
    def __init__(self, credo, title='Data display'):
        ...
        for flaglabel, flagname in _errorflags:
            ...
            self._flagbuttons[flagname].override_background_color(
                Gtk.StateFlags.ACTIVE, Gdk.RGBA(1, 0, 0, 1))
            self._flagbuttons[flagname].override_background_color(
                Gtk.StateFlags.ACTIVE | Gtk.StateFlags.NORMAL, Gdk.RGBA(1, 0, 0, 1))
            self._flagbuttons[flagname].connect(
                'toggled', self._on_flag, flagname)
        ...
        self.plot2d.logo = os.path.join(
            self.credo.subsystems['Files'].rootpath, 'credo_logo.png')
    ...
    def on_data_reduction_finished(self, ssdr, fsn, header, button, fsn_to_wait_for):
        ...
        exposure = ssdr.load_exposure(fsn)
        self.plot1d.add_curve_with_errorbar(
            exposure.radial_average(), label='Reduced: ' + str(exposure.header))
    ...
    def do_data_reduction(self, button):
        if self.plot2d.get_exposure() is None:
            md = Gtk.MessageDialog(self, Gtk.DialogFlags.DESTROY_WITH_PARENT | Gtk.DialogFlags.MODAL,
                                   Gtk.MessageType.ERROR, Gtk.ButtonsType.OK, 'Please open a file first!')
            md.run()
            md.destroy()
            del md
            return
        ...
        if not os.path.split(self.plot2d.get_exposure()['FileName'])[1].startswith(ssdr.filebegin):
            md = Gtk.MessageDialog(self, Gtk.DialogFlags.DESTROY_WITH_PARENT | Gtk.DialogFlags.MODAL, Gtk.MessageType.ERROR,
                                   Gtk.ButtonsType.OK, 'Cannot do data reduction on exposure. File must begin with %s' % ssdr.filebegin)
            md.run()
            md.destroy()
            del md
            return
        ...
        self._dr_connid = ssdr.connect(
            'done', self.on_data_reduction_finished, button, self.plot2d.get_exposure()['FSN'])
    ...
    def _exposure_open(self, eselector, ex):
        ...
        try:
            ...
            self.plot1d.gca().text(0.5, 0.5, 'Cannot do radial average:\n' + str(see),
                                   ha='center', va='center', transform=self.plot1d.gca().transAxes)
        else:
    ...
    def _editmask(self, widget):
        maskmaker = sasgui.maskmaker.MaskMaker(
            matrix=self.plot2d.get_exposure())
        resp = maskmaker.run()
