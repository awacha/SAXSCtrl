...
class InstrumentSetup(gtk.Dialog):
    def __init__(self, credo, title='Instrument parameters', parent=None, flags=gtk.DIALOG_DESTROY_WITH_PARENT, buttons=(gtk.STOCK_OK, gtk.RESPONSE_OK, gtk.STOCK_APPLY, gtk.RESPONSE_APPLY, gtk.STOCK_CANCEL, gtk.RESPONSE_CANCEL)):
        ...
        tab.attach(l, 0, 1, row, row + 1, gtk.FILL, gtk.FILL)
        self.dist_entry = gtk.SpinButton(gtk.Adjustment(1000, 0, 10000, 0.1, 1), digits=4)
        tab.attach(self.dist_entry, 1, 2, row, row + 1)
        row += 1
        
        l = gtk.Label(u'Beam position (vertical):'); l.set_alignment(0, 0.5)
        tab.attach(l, 0, 1, row, row + 1, gtk.FILL, gtk.FILL)
        self.beamposx_entry = gtk.SpinButton(gtk.Adjustment(308.298, -1000, 1000, 0.1, 1), digits=4)
        tab.attach(self.beamposx_entry, 1, 2, row, row + 1)
        row += 1

        l = gtk.Label(u'Beam position (horizontal):'); l.set_alignment(0, 0.5)
        tab.attach(l, 0, 1, row, row + 1, gtk.FILL, gtk.FILL)
        self.beamposy_entry = gtk.SpinButton(gtk.Adjustment(243.63, -1000, 1000, 0.1, 1), digits=4)
        tab.attach(self.beamposy_entry, 1, 2, row, row + 1)
        row += 1

        l = gtk.Label(u'Pixel size (\u03bcm):'); l.set_alignment(0, 0.5)
        tab.attach(l, 0, 1, row, row + 1, gtk.FILL, gtk.FILL)
        self.pixelsize_entry = gtk.SpinButton(gtk.Adjustment(172, 0, 5000, 1, 10), digits=2)
    ...
    def on_response(self, dialog, respid):
        if respid == gtk.RESPONSE_OK or respid == gtk.RESPONSE_APPLY:
