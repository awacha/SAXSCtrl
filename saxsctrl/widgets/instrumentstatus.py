...
class InstrumentStatusLabel(Gtk.Box):
    ...
    def __init__(self, instrument, propertyname, label=None, formatter=None, colourer=None):
        ...
        Gtk.Box.__init__(
            self, orientation=Gtk.Orientation.VERTICAL, homogeneous=True)
        ...
        if self._instrument.is_instrumentproperty(self._propertyname):
            self._connection = self._instrument.connect(
                'instrumentproperty-notify', self._on_instrumentproperty_notify)
        else:
            self._connection = self._instrument.connect(
                'notify::' + self._propertyname, self._on_property_notify)
    ...
    def _on_instrumentproperty_notify(self, instrument, propertyname):
        if propertyname != self._propertyname:
            return False
        try:
            value, category = instrument.get_instrument_property(
                propertyname)[::2]
    ...
    def _default_colourer(value, category):
        if category in [InstrumentPropertyCategory.ERROR, InstrumentPropertyCategory.NO]:
            ...
            raise NotImplementedError(
                'Instrument property category "%s" is unknown for InstrumentStatusLabel.' % category)
    ...
    def _on_property_notify(self, instrument, prop):
        try:
            ...
            self._eventbox.override_background_color(
                Gtk.StateFlags.NORMAL, Gdk.RGBA(*Gdk.color_parse('gray').to_floats()))
            self._valuelabel.set_text('UNKNOWN')
        else:
            if self._colourer is None:
                self._eventbox.override_background_color(
                ...
                self._eventbox.override_background_color(
                    Gtk.StateFlags.NORMAL, self._colourer(value, None))
            self._valuelabel.set_text(self._formatter(value))
        return True
    ...
    def refresh(self):
        if self._instrument.is_instrumentproperty(self._propertyname):
            self._on_instrumentproperty_notify(
                self._instrument, self._propertyname)
        else:
...
class InstrumentStatus(Gtk.Frame):
    ...
    def add_label(self, propertyname, label=None, formatter=None, colourer=None):
        newlabel = InstrumentStatusLabel(
            self.instrument, propertyname, label, formatter, colourer)
