...
class MotorMonitorFrame(Gtk.Frame):
    ...
    def __init__(self, credo):
        ...
        vb.pack_start(Gtk.Label(
            label='To move a motor, double-click on the corresponding row.'), False, False, 0)

        self.credo = credo
        self.credo.subsystems['Motors'].connect(
            'motor-report', self.on_motor_move)
        self.credo.subsystems['Motors'].connect(
            'motors-changed', self.on_motors_changed)
        self.credo.subsystems['Motors'].connect(
            'motor-settings-changed', self.on_motor_settings_changed)
        self.credo.subsystems['Motors'].connect(
            'motor-limit', self.on_motor_limit)
        self.motorlist = Gtk.ListStore(GObject.TYPE_STRING, GObject.TYPE_STRING, GObject.TYPE_STRING,
                                       GObject.TYPE_STRING, GObject.TYPE_BOOLEAN, GObject.TYPE_BOOLEAN, GObject.TYPE_INT)
        self.motorview = Gtk.TreeView(self.motorlist)
        self.motorview.append_column(
            Gtk.TreeViewColumn('Name', Gtk.CellRendererText(), text=0))
        self.motorview.append_column(
            Gtk.TreeViewColumn('Alias', Gtk.CellRendererText(), text=1))
        self.motorview.append_column(
            Gtk.TreeViewColumn('Position', Gtk.CellRendererText(), text=2))
        self.motorview.append_column(
            Gtk.TreeViewColumn('Speed', Gtk.CellRendererText(), text=3))
        ...
        self.motorview.append_column(
            Gtk.TreeViewColumn('Left limit', crt, active=4))
        crt = Gtk.CellRendererToggle()
        crt.set_activatable(False)
        self.motorview.append_column(
            Gtk.TreeViewColumn('Right limit', crt, active=5))
        self.motorview.append_column(
            Gtk.TreeViewColumn('Load', Gtk.CellRendererText(), text=6))
    ...
    def on_motors_changed(self, ssmot):
        ...
        for mot in ssmot.get_motors():
            self.motorlist.append(
                (mot.name, mot.alias, '', '', False, False, 0))
...
class SaveMotorsDialog(Gtk.Dialog):
    ...
    def __init__(self, title='Save motor configuration', parent=None):
        Gtk.Dialog.__init__(self, title, parent, destroy_with_parent=True, modal=True, buttons=(
            'Save', Gtk.ResponseType.OK, 'Cancel', Gtk.ResponseType.CANCEL))
        hb = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL)
        self.get_content_area().pack_start(hb, False, False, 0)
        l = Gtk.Label(label='Name of configuration:')
        ...
        self._name_entry.set_text(
            'Motor state at ' + str(datetime.datetime.now()))
        self._treemodel = Gtk.ListStore(
            GObject.TYPE_BOOLEAN, GObject.TYPE_STRING, GObject.TYPE_STRING, GObject.TYPE_FLOAT, GObject.TYPE_OBJECT)
        self._treeview = Gtk.TreeView(self._treemodel)
        cr = Gtk.CellRendererToggle()
        self._treeview.append_column(
            Gtk.TreeViewColumn('Include?', cr, active=0))
        cr.set_property('activatable', True)
        cr.connect('toggled', self._tobeincluded_toggled)
        self._treeview.append_column(
            Gtk.TreeViewColumn('Motor name', Gtk.CellRendererText(), text=1))
        self._treeview.append_column(
            Gtk.TreeViewColumn('Motor alias', Gtk.CellRendererText(), text=2))
    ...
    def _load_from_credo(self, credo):
        ...
        for m in credo.subsystems['Motors'].get_motors():
            ...
            self._treemodel.append([True, m.name, m.alias, pos, Gtk.Adjustment(
                pos, m.get_parameter('soft_left'), m.get_parameter('soft_right'), 1, 10)])
        return True

    def save_state(self, cp):
...
class MotorMonitor(ToolDialog):
    ...
    def __init__(self, credo, title='Motor positions'):
        ToolDialog.__init__(
            self, credo, title, buttons=('Close', Gtk.ResponseType.CLOSE, 'Refresh', 1))
        ...
        # name, motornames list, spinner enable, spinner pulse
        self._motorstates_list = Gtk.ListStore(
            GObject.TYPE_STRING, GObject.TYPE_STRING, GObject.TYPE_BOOLEAN, GObject.TYPE_UINT)
        self._savedmotorstates_view = Gtk.TreeView(self._motorstates_list)
        sw.add(self._savedmotorstates_view)
        self._savedmotorstates_view.append_column(
            Gtk.TreeViewColumn('Name', Gtk.CellRendererText(), text=0))
        self._savedmotorstates_view.append_column(
            Gtk.TreeViewColumn('Motors saved', Gtk.CellRendererText(), text=1))
        cr = Gtk.CellRendererSpinner()
        self._savedmotorstates_view.append_column(
            Gtk.TreeViewColumn('', cr, active=2, pulse=3))

        self._savedmotorstates_view.get_selection().set_mode(
            Gtk.SelectionMode.SINGLE)
        self._savedmotorstates_view.set_rules_hint(True)
        self._savedmotorstates_view.set_headers_visible(True)
        self._savedmotorstates_view.connect(
        ...
        b.set_image(
            Gtk.Image.new_from_icon_name('system-run', Gtk.IconSize.BUTTON))
        vbb.pack_start(b, True, True, 0)
    ...
    def _reload_from_file(self):
        ...
        cp.read(
            os.path.join(self.credo.subsystems['Files'].configpath, 'motorconfigs.conf'))
        for s in sorted(cp.sections()):
            self._motorstates_list.append(
                [s, ', '.join(cp.options(s)), False, 0])
        del cp
    ...
    def _on_row_activated(self, treeview, path, treeviewcolumn):
        ...
        self._movetostoredconfig_conn = ssm.connect(
            'idle', self._movement_finished, path)
        self._movetostoredconfig_pulser = GLib.timeout_add(
            100, self._movement_pulse_spinner, path)
        row[2] = True
...
class EntryPair(GObject.GObject):
    ...
    __gsignals__ = {'motparentry-changed':
    ...
    def __init__(self, stateholder, motpar):
        ...
        self.rawentry = Gtk.SpinButton(adjustment=Gtk.Adjustment(
            0, min_, max_, 1, 10), digits=[4, 0][self._motpar.isinteger])
        self.rawentry.set_numeric(True)
        self.physentry = Gtk.SpinButton(adjustment=Gtk.Adjustment(
            0, -1e6, 1e6, 1, 10), digits=[4, 0][self._motpar.isinteger])
    ...
    def _raw_changed(self, rawentry):
        ...
        physval = self._motpar.to_phys(
            self.rawentry.get_value(), self._stateholder._stateparams)
        if not self._motpar.validate(physval, self._stateholder._stateparams):
            self.rawentry.override_background_color(
                Gtk.StateType.NORMAL, Gdk.RGBA(*Gdk.color_parse('red').to_floats()))
        else:
    ...
    def _phys_changed(self, physentry):
        ...
        if not self._motpar.validate(self.physentry.get_value(), self._stateholder._stateparams):
            self.physentry.override_background_color(
                Gtk.StateType.NORMAL, Gdk.RGBA(*Gdk.color_parse('red').to_floats()))
        else:
            self.physentry.override_background_color(
                Gtk.StateType.NORMAL, None)
        if hasattr(self, 'setting_phys_because_raw_changed'):
            del self.setting_phys_because_raw_changed
            return
        rawval = self._motpar.to_raw(
            self.physentry.get_value(), self._stateholder._stateparams)
    ...
    def todevice(self):
        ...
        self._stateholder.set_parameter(
            self._motpar.name, self.rawentry.get_value(), raw=True)
...
class MotorParamSpinButton(Gtk.SpinButton):
    ...
    def __init__(self, stateholder, motpar):
        ...
        if motpar.rawmaximum is not None:
            ...
            max_ = 1e12
        Gtk.SpinButton.__init__(self, adjustment=Gtk.Adjustment(
            0, min_, max_, 1, 10), digits=[3, 0][motpar.isinteger])
        self.set_hexpand(True)
    ...
    def fromdevice(self):
        self.set_value(
            self._stateholder.get_parameter(self._motpar.name, raw=True))
        self.update()

    def todevice(self):
        self._stateholder.set_parameter(
            self._motpar.name, self.get_value(), raw=True)
...
class MotorDriver(ToolDialog):
    ...
    def __init__(self, credo, motorname, title=None):
        ...
        ToolDialog.__init__(self, credo, title, buttons=(
            'Close', Gtk.ResponseType.CLOSE, 'Refresh', 1, 'Apply', Gtk.ResponseType.APPLY, 'Save', 2))
        ...
        f.add(grid)
        row = 0
        l = Gtk.Label(label='Current position:')
        l.set_halign(Gtk.Align.START)
        l.set_valign(Gtk.Align.CENTER)
        grid.attach(l, 0, row, 1, 1)

        self._poslabel = Gtk.Label()
        self._poslabel.set_halign(Gtk.Align.START)
        self._poslabel.set_valign(Gtk.Align.CENTER)
        self._poslabel.set_label(
            '%.03f' % self.motor.get_parameter('Current_position'))
        grid.attach(self._poslabel, 1, row, 1, 1)
        row += 1
        l = Gtk.Label(label='Move ' + motorname + ' to:')
        l.set_halign(Gtk.Align.START)
        l.set_valign(Gtk.Align.CENTER)
        grid.attach(l, 0, row, 1, 1)
        hb = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL)
        ...
        hb.set_hexpand(True)
        self._moveto_entry = Gtk.SpinButton(
            adjustment=Gtk.Adjustment(0, -1e20, 1e20, 0.1, 1), digits=3)
        ...
        self._relative_cb.set_halign(Gtk.Align.START)
        ...
        eb.override_background_color(
            Gtk.StateType.NORMAL, Gdk.RGBA(1, 0, 0, 1))
        ...
        hb.pack_start(Gtk.Image.new_from_icon_name(
            'dialog-warning', size=Gtk.IconSize.MENU), False, False, 3)
        ...
        l = Gtk.Label(label='Driver units')
        grid.attach(l, 1, row, 1, 1)
        l = Gtk.Label(label='Physical units')
        grid.attach(l, 2, row, 1, 1)
        row += 1

        l = Gtk.Label(label='Max RMS current (A):')
        l.set_halign(Gtk.Align.START)
        l.set_valign(Gtk.Align.CENTER)
        grid.attach(l, 0, row, 1, 1)
        ep = EntryPair(self, [
                       m for m in self.motor.driver().motor_params if m.name == 'Max_RMS_current'][0])
        self._entries.append(ep)
        grid.attach(ep.rawentry, 1, row, 1, 1)
        grid.attach(ep.physentry, 2, row, 1, 1)
        row += 1

        l = Gtk.Label(label='Standby RMS current (A):')
        l.set_halign(Gtk.Align.START)
        l.set_valign(Gtk.Align.CENTER)
        grid.attach(l, 0, row, 1, 1)
        ep = EntryPair(self, [m for m in self.motor.driver(
        ).motor_params if m.name == 'Standby_RMS_current'][0])
        self._entries.append(ep)
        grid.attach(ep.rawentry, 1, row, 1, 1)
        grid.attach(ep.physentry, 2, row, 1, 1)
        row += 1

        l = Gtk.Label(label='Freewheeling delay (msec):')
        l.set_halign(Gtk.Align.START)
        l.set_valign(Gtk.Align.CENTER)
        grid.attach(l, 0, row, 1, 1)
        sb = MotorParamSpinButton(self, [m for m in self.motor.driver(
        ).motor_params if m.name == 'Freewheeling_delay'][0])
        ...
        l = Gtk.Button('Calibrate pos to:')
        l.set_halign(Gtk.Align.START)
        l.set_valign(Gtk.Align.CENTER)
        grid.attach(l, 0, row, 1, 1)
        ep = EntryPair(self, [
                       m for m in self.motor.driver().motor_params if m.name == 'Current_position'][0])
        grid.attach(ep.rawentry, 1, row, 1, 1)
        grid.attach(ep.physentry, 2, row, 1, 1)
        ...
        l = Gtk.Label(label='Left software limit:')
        l.set_halign(Gtk.Align.START)
        l.set_valign(Gtk.Align.CENTER)
        grid.attach(l, 0, row, 1, 1)
        ep = EntryPair(
            self, [m for m in self.motor.driver().motor_params if m.name == 'soft_left'][0])
        self._entries.append(ep)
        grid.attach(ep.rawentry, 1, row, 1, 1)
        grid.attach(ep.physentry, 2, row, 1, 1)
        row += 1

        l = Gtk.Label(label='Right software limit:')
        l.set_halign(Gtk.Align.START)
        l.set_valign(Gtk.Align.CENTER)
        grid.attach(l, 0, row, 1, 1)
        ep = EntryPair(
            self, [m for m in self.motor.driver().motor_params if m.name == 'soft_right'][0])
        self._entries.append(ep)
        grid.attach(ep.rawentry, 1, row, 1, 1)
        grid.attach(ep.physentry, 2, row, 1, 1)
        row += 1

        cb = MotorParamCheckButton(self, [m for m in self.motor.driver(
        ).motor_params if m.name == 'Left_limit_disable'][0], label='Disable left limit switch')
        grid.attach(cb, 0, row, 3, 1)
        self._entries.append(cb)
        row += 1

        cb = MotorParamCheckButton(self, [m for m in self.motor.driver(
        ).motor_params if m.name == 'Right_limit_disable'][0], label='Disable right limit switch')
        grid.attach(cb, 0, row, 3, 1)
        self._entries.append(cb)
        row += 1
        ...
        l = Gtk.Label(label='Full step physical size:')
        l.set_halign(Gtk.Align.START)
        l.set_valign(Gtk.Align.CENTER)
        grid.attach(l, 0, row, 1, 1)
        sb = MotorParamSpinButton(
            self, [m for m in self.motor.driver().motor_params if m.name == 'step_to_cal'][0])
        self._entries.append(sb)
        grid.attach(sb, 1, row, 2, 1)
        row += 1

        l = Gtk.Label(label='Maximum speed:')
        l.set_halign(Gtk.Align.START)
        l.set_valign(Gtk.Align.CENTER)
        grid.attach(l, 0, row, 1, 1)
        ep = EntryPair(
            self, [m for m in self.motor.driver().motor_params if m.name == 'Max_speed'][0])
        self._entries.append(ep)
        grid.attach(ep.rawentry, 1, row, 1, 1)
        grid.attach(ep.physentry, 2, row, 1, 1)
        row += 1

        l = Gtk.Label(label='Maximum acceleration:')
        l.set_halign(Gtk.Align.START)
        l.set_valign(Gtk.Align.CENTER)
        grid.attach(l, 0, row, 1, 1)
        ep = EntryPair(
            self, [m for m in self.motor.driver().motor_params if m.name == 'Max_accel'][0])
        self._entries.append(ep)
        grid.attach(ep.rawentry, 1, row, 1, 1)
        grid.attach(ep.physentry, 2, row, 1, 1)
        row += 1

        l = Gtk.Label(label='Logarithmic microstep resolution:')
        l.set_halign(Gtk.Align.START)
        l.set_valign(Gtk.Align.CENTER)
        grid.attach(l, 0, row, 1, 1)
        sb = MotorParamSpinButton(
            self, [m for m in self.motor.driver().motor_params if m.name == 'Ustep_resol'][0])
        self._entries.append(sb)
        grid.attach(sb, 1, row, 2, 1)
        row += 1

        l = Gtk.Label(label='Logarithmic pulse divisor:')
        l.set_halign(Gtk.Align.START)
        l.set_valign(Gtk.Align.CENTER)
        grid.attach(l, 0, row, 1, 1)
        sb = MotorParamSpinButton(
            self, [m for m in self.motor.driver().motor_params if m.name == 'Pulse_div'][0])
        self._entries.append(sb)
        grid.attach(sb, 1, row, 2, 1)
        row += 1

        l = Gtk.Label(label='Logarithmic ramp divisor:')
        l.set_halign(Gtk.Align.START)
        l.set_valign(Gtk.Align.CENTER)
        grid.attach(l, 0, row, 1, 1)
        sb = MotorParamSpinButton(
            self, [m for m in self.motor.driver().motor_params if m.name == 'Ramp_div'][0])
        self._entries.append(sb)
        grid.attach(sb, 1, row, 2, 1)
        row += 1
    ...
    def on_motparentry_changed(self, mpe, motpar):
        ...
        for e in [e for e in self._entries if (motpar.name in e._motpar.depends) and (hasattr(e, '_raw_changed'))]:
            logger.debug('Recalculating %s because %s changed.' %
                         (e._motpar.name, motpar.name))
    ...
    def refresh_settings(self):
        try:
            logger.debug(
                "Refreshing settings of motor driver for %s" % str(self.motor))
            ...
            md = Gtk.MessageDialog(self, Gtk.DialogFlags.DESTROY_WITH_PARENT |
                                   Gtk.DialogFlags.MODAL, Gtk.MessageType.ERROR, Gtk.ButtonsType.OK, str(me))
    ...
    def get_parameter(self, name, raw=False):
        if raw:
            ...
            motpar = [
                m for m in self.motor.driver().motor_params if m.name == name][0]
    ...
    def on_moveto(self, widget):
        ...
        self.motorconns.append(
            self.motor.connect('motor-report', self.on_motor_report))
        self.motorconns.append(
            self.motor.connect('motor-stop', self.on_motor_stop))
        try:
            self.motor.moveto(
                float(self._moveto_entry.get_value()), False, self._relative_cb.get_active())
    ...
    def do_response(self, respid):
        if respid == 1:
            ...
            md = Gtk.MessageDialog(self, Gtk.DialogFlags.DESTROY_WITH_PARENT | Gtk.DialogFlags.MODAL, Gtk.MessageType.QUESTION,
                                   Gtk.ButtonsType.YES_NO, 'Do you really want to save the current motor state to the driver\'s EEPROM?')
            ...
            md = Gtk.MessageDialog(self, Gtk.DialogFlags.DESTROY_WITH_PARENT | Gtk.DialogFlags.MODAL,
                                   Gtk.MessageType.QUESTION, Gtk.ButtonsType.YES_NO, 'Do you really want to apply the following changes?')
