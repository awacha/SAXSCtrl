...
class ScanGraph(ToolDialog):
    ...
    is_recording = GObject.property(
        type=bool, default=False, blurb='Scan running')
    ...
    def __init__(self, scan, credo, title='Scan results'):
        ...
        ToolDialog.__init__(self, credo, title, buttons=(
            'Close', Gtk.ResponseType.CLOSE, 'Derivative', self.RESPONSE_DERIVATIVE, 'Integrate', self.RESPONSE_INTEGRATE))
...
#        vb.pack_start(tab, False, True, 0)
        ...
        b.set_tooltip_text(
            'Fit a Lorentzian peak to the zoomed portion of the currently selected signal')
        self.figtoolbar.insert(b, self.figtoolbar.get_n_items() - 2)
        b.connect('clicked', self.on_fitpeak)

        self.scan = scan
        self.datacols = [
            c for c in self.scan.columns() if c != self.scan.get_dataname('x')]
        ...
        ls = Gtk.ListStore(
            GObject.TYPE_STRING, GObject.TYPE_BOOLEAN, GObject.TYPE_FLOAT)
        self.scalertreeview = Gtk.TreeView(ls)
        cr = Gtk.CellRendererText()
        self.scalertreeview.append_column(
            Gtk.TreeViewColumn('Name', cr, text=0))
        cr = Gtk.CellRendererToggle()
        cr.set_activatable(True)
        cr.connect('toggled', self.on_cell_toggled)
        self.scalertreeview.append_column(
            Gtk.TreeViewColumn('Visible', cr, active=1))
        cr = Gtk.CellRendererSpin()
        cr.set_property('adjustment', Gtk.Adjustment(
            value=0, lower=-1e9, upper=1e9, step_increment=1, page_increment=10))
    ...
    def _on_motor_to_cursor(self, *buttons):
        try:
            self._movement_connection = self.motor.connect(
                'idle', self._on_motor_idle, *buttons)
            ...
            md = Gtk.MessageDialog(self, Gtk.DialogFlags.DESTROY_WITH_PARENT | Gtk.DialogFlags.MODAL,
                                   Gtk.MessageType.ERROR, Gtk.ButtonsType.OK, 'Cannot move to cursor')
    ...
    def _on_motor_to_peak(self, *buttons):
        if not hasattr(self, '_lastpeakpos'):
            md = Gtk.MessageDialog(self, Gtk.DialogFlags.DESTROY_WITH_PARENT | Gtk.DialogFlags.MODAL,
                                   Gtk.MessageType.ERROR, Gtk.ButtonsType.OK, 'Cannot move to peak')
            md.set_title(
                'Please do a peak fit before trying to move the motor to the peak position!')
            md.run()
            md.destroy()
            del md
            return
    ...
    def do_notify(self, prop):
        if prop.name == 'is-recording':
            if not self.is_recording:
                ...
                self._cursor_scale.grab_focus()
                x = self.scan[self.xname]
                if not len(x):
                    return
                self._cursor_scale.set_range(x.min(), x.max())
                xwid = x.max() - x.min()
                self._cursor_scale.set_increments(
                    xwid / (len(x) - 1.), 10 * xwid / (len(x) - 1.))
    ...
    def do_response(self, respid):
        if respid in (Gtk.ResponseType.CLOSE, Gtk.ResponseType.DELETE_EVENT):
            ...
            sg = ScanGraph(
                self.scan.diff(), self.credo, title='Derivative of ' + self.get_title())
            sg.set_scalers([(x[0], x[1], x[2])
                            for x in self.scalertreeview.get_model()])
            sg.redraw_scan()
            sg.show_all()
        elif respid == self.RESPONSE_INTEGRATE:
            sg = ScanGraph(
                self.scan.integrate(), self.credo, title='Integrated ' + self.get_title())
            sg.set_scalers([(x[0], x[1], x[2])
                            for x in self.scalertreeview.get_model()])
            sg.redraw_scan()
            sg.show_all()
    ...
    def move_cursor(self, to):
        ...
        try:
            ...
            x = self.scan[self.xname]
            desired_index = np.interp(
                to, x, np.arange(len(self.scan)), 0, len(self.scan) - 1)
    ...
    def redraw_scan(self, full=False):
        ...
        if full:
            ...
            for col, color in zip(self.datacols, itertools.cycle(matplotlib.rcParams['axes.color_cycle'])):
                ...
                self._lines.extend(
                    self.gca().plot(x, self.scan[col] * scale, '.-', color=color, label=col))
                self._cursors[col].set_xdata(x[self._cursor_at])
                self._cursors[col].set_ydata(
...
#            self.fig.gca().axis('tight')
        ...
        if self._show2d_check.get_active() and ('FSN' in self.scan.columns()):
            ...
            exposure = sastool.classes.SASExposure(ssf.get_exposureformat('scn') % self.scan['FSN'][self._cursor_at], dirs=[
                                                   ssf.scanpath, ssf.imagespath, ssf.parampath] + sastool.misc.find_subdirs(ssf.maskpath))
        ...
        for c in self.currvallabels:
            self.currvallabels[c].set_label(
                '%f' % self.scan[c][self._cursor_at])
    ...
    def on_fitpeak(self, button):
        ...
        signalname = model[iter_][0]
        scale = [m[2]
                 for m in self.scalertreeview.get_model() if m[0] == signalname][0]
        curve = sastool.GeneralCurve(
            self.scan[self.xname], self.scan[signalname] * scale)
        curve = curve.trim(*(self.gca().axis()))
        try:
            if not len(curve):
                raise TypeError()
            pos, hwhm, baseline, amplitude = sastool.misc.findpeak_single(
                curve.x, curve.y, curve='Lorentz')
        except TypeError:
            md = Gtk.MessageDialog(self, Gtk.DialogFlags.DESTROY_WITH_PARENT |
                                   Gtk.DialogFlags.MODAL, Gtk.MessageType.ERROR, Gtk.ButtonsType.OK, 'Fitting error')
            md.set_title(
                'Please make sure you selected the correct signal for fitting.')
            md.run()
            md.destroy()
            del md
            return
        xfitted = np.linspace(curve.x.min(), curve.x.max(), 5 * len(curve.x))
        fitted = sastool.GeneralCurve(
            xfitted, amplitude * hwhm ** 2 / (hwhm ** 2 + (pos - xfitted) ** 2) + baseline)
        if hasattr(self, '_fittedline'):
            self._fittedline.set_xdata(fitted.x)
            self._fittedline.set_ydata(fitted.y)
        else:
            self._fittedline = fitted.plot(
                'r-', axes=self.gca(), label='Peak of %s at: ' % signalname + str(pos))[0]
        if hasattr(self, '_peakpostext') and self._peakpostext is not None:
            self._peakpostext.set_text('Peak at: ' + str(pos))
        else:
            self._peakpostext = self.text(float(pos), curve.interpolate(
                float(pos)), 'Peak at: ' + str(pos), ha='left', va='top')
...
class ImagingGraph(ToolDialog):
    ...
    def __init__(self, scan, title='Imaging results', extent=None):
        ...
        ls = Gtk.ListStore(
            GObject.TYPE_STRING, GObject.TYPE_BOOLEAN, GObject.TYPE_STRING, GObject.TYPE_FLOAT)
        self.scalertreeview = Gtk.TreeView(ls)
        cr = Gtk.CellRendererText()
        self.scalertreeview.append_column(
            Gtk.TreeViewColumn('Name', cr, text=0))
        ...
        l = Gtk.Label(label='Number of columns:')
        l.set_halign(Gtk.Align.START)
        l.set_valign(Gtk.Align.CENTER)
        hb1.pack_start(l, False, False, 0)
        self._ncol_sb = Gtk.SpinButton(
            adjustment=Gtk.Adjustment(4, 1, 100, 1, 10), digits=0)
        self._ncol_sb.set_value(2)
        self._ncol_sb.connect(
            'value-changed', lambda sb: self.redraw_scan(True))
    ...
    def redraw_scan(self, full=False):
        ...
        if full:
            ...
            for i, col in enumerate(self._visible_cols):
                ...
                self._axes.append(
                    self.gcf().add_subplot(Nsubplotrows, Nsubplotcols, i + 1))
                img = self.scan.get_image(col) * factor
                logger.debug('ImagingGraph.redraw_scan(True): column %s, image dimensions: (%d, %d). NaNs: %d' % (
                    col, img.shape[0], img.shape[1], (-np.isfinite(img)).sum()))
                if scale.upper() == 'LOG':
                    img = np.log10(img)
                ...
                if self._extent is None:
                    ...
                    extent = self._extent
                self._axes[-1].imshow(img, interpolation='nearest',
            ...
            for col, ax in zip(self._visible_cols, self._axes):
                ...
                if scale.upper() == 'LOG':
                    ...
                    img = np.sqrt(img)
                logger.debug('ImagingGraph.redraw_scan(Fals): column %s, image dimensions: (%d, %d). NaNs: %d' % (
                    col, img.shape[0], img.shape[1], (-np.isfinite(img)).sum()))
        ...
        if scalerlist is None:
            ...
            for name, visible, scaling, factor in scalerlist:
                if scaling.upper().startswith('LIN'):
                    ...
                    raise ValueError(
                        'Invalid scaling type for column %s: %s' % (name, scaling))
