...
class ExposureBrowserDialog(Gtk.Dialog):
    ...
    def __init__(self, credo, fileprefix, ndigits, title='Open exposure...', parent=None, flags=Gtk.DialogFlags.DESTROY_WITH_PARENT):
        Gtk.Dialog.__init__(self, title, parent, flags, buttons=(
            'Cancel', Gtk.ResponseType.CANCEL, 'Refresh', Gtk.ResponseType.APPLY, 'Open', Gtk.ResponseType.OK))
        self.credo = credo
        ...
        self.fsn_start_cb = Gtk.CheckButton('Starting FSN:')
        self.fsn_start_cb.set_halign(Gtk.Align.START)
        self.fsn_start_cb.set_valign(Gtk.Align.CENTER)
        tab.attach(self.fsn_start_cb, 0, 1, row, row + 1,
                   Gtk.AttachOptions.FILL, Gtk.AttachOptions.FILL)
        self.fsn_start_entry = Gtk.SpinButton(
            adjustment=Gtk.Adjustment(1, 0, 9999999999, 1, 10), digits=0)
        tab.attach(self.fsn_start_entry, 1, 2, row, row + 1)
        self.fsn_start_cb.connect(
            'toggled', self._checkbutton_with_entry, self.fsn_start_entry)
        self._checkbutton_with_entry(self.fsn_start_cb, self.fsn_start_entry)
        row += 1

        self.fsn_end_cb = Gtk.CheckButton('Ending FSN:')
        self.fsn_end_cb.set_halign(Gtk.Align.START)
        self.fsn_end_cb.set_valign(Gtk.Align.CENTER)
        tab.attach(self.fsn_end_cb, 0, 1, row, row + 1,
                   Gtk.AttachOptions.FILL, Gtk.AttachOptions.FILL)
        self.fsn_end_entry = Gtk.SpinButton(
            adjustment=Gtk.Adjustment(1, 0, 9999999999, 1, 10), digits=0)
        tab.attach(self.fsn_end_entry, 1, 2, row, row + 1)
        self.fsn_end_cb.connect(
            'toggled', self._checkbutton_with_entry, self.fsn_end_entry)
        ...
        for i, coltitle in enumerate(['FSN', 'Title', 'Owner', 'Distance', 'Exp. time']):
            ...
            tvc.connect(
                'clicked', self._tvc_column_clicked, tvc.get_title(), i + 1)
        ...
        self.treeview.connect(
            'row-activated', lambda tv, path, column: self.response(Gtk.ResponseType.OK))
    ...
    def reload(self, *args, **kwargs):
        ...
        if self.fsn_start_cb.get_active():
            ...
            minfsn = self.credo.subsystems['Files'].get_first_fsn(
                self.credo.subsystems['Files'].get_format_re(self.fileprefix, self.ndigits))
        if self.fsn_end_cb.get_active():
            maxfsn = self.fsn_end_entry.get_value_as_int()
        else:
            maxfsn = self.credo.subsystems['Files'].get_next_fsn(
                self.credo.subsystems['Files'].get_format_re(self.fileprefix, self.ndigits)) - 1
        if minfsn is None or maxfsn is None:
            return
        headers = sastool.classes.SASHeader(self.credo.subsystems['Files'].get_headerformat(
            self.fileprefix, self.ndigits), list(range(minfsn, maxfsn + 1)), dirs=datadirs, error_on_not_found=False)
...
class ExposureSelector(Gtk.Frame):
    ...
    def __init__(self, credo, filebegin='crd', ndigits=5, loadtype='All'):
        ...
        l = Gtk.Label(label='Digits in filename:')
        l.set_halign(Gtk.Align.START)
        l.set_valign(Gtk.Align.CENTER)
        grid.attach(l, 0, row, 1, 1)
        self._digits_sb = Gtk.SpinButton(
            adjustment=Gtk.Adjustment(5, 1, 10, 1, 10), digits=0)
        ...
        l = Gtk.Label(label='File sequence number:')
        l.set_halign(Gtk.Align.START)
        l.set_valign(Gtk.Align.CENTER)
        grid.attach(l, 0, row, 1, 1)
        self._fsn_entry = Gtk.SpinButton(
            adjustment=Gtk.Adjustment(value=1, lower=0, upper=9999999999, step_increment=1, page_increment=10), digits=0)
        ...
        b = Gtk.Button()
        b.set_image(
            Gtk.Image.new_from_icon_name('edit-find', Gtk.IconSize.BUTTON))
        hb1.pack_start(b, False, False, 0)
        b.connect('clicked', lambda b: self._browse())
        row += 1

        self._mask_override_cb = Gtk.CheckButton('Override mask with:')
        self._mask_override_cb.set_halign(Gtk.Align.START)
        ...
        self._mask_override_cb.connect(
            'toggled', lambda cb: self._maskentry.set_sensitive(cb.get_active()))
    ...
    def _load_first(self):
        self._fsn_entry.set_value(self.credo.subsystems['Files'].get_first_fsn(self.credo.subsystems[
                                  'Files'].get_format_re(self._fileprefix_combo.get_active_text(), self._digits_sb.get_value_as_int())) - 1)
        # self._load_current()
        ...
        try:
            if self.loadtype == 0:  # All
                ...
                raise NotImplementedError(
                    'Unknown loadtype: ' + str(self.loadtype))
            ...
            md = Gtk.MessageDialog(self.get_toplevel(), Gtk.DialogFlags.DESTROY_WITH_PARENT |
                                   Gtk.DialogFlags.MODAL, Gtk.MessageType.ERROR, Gtk.ButtonsType.OK, 'Error reading file: ' + str(ioe))
            md.run()
            md.destroy()
            del md
    ...
    def _load_last(self):
        self._fsn_entry.set_value(self.credo.subsystems['Files'].get_next_fsn(self.credo.subsystems[
                                  'Files'].get_format_re(self._fileprefix_combo.get_active_text(), self._digits_sb.get_value_as_int())) - 1)
        # self._load_current()

    def _browse(self):
        dlg = ExposureBrowserDialog(self.credo, self._fileprefix_combo.get_active_text(
        ), self._digits_sb.get_value_as_int(), parent=self.get_toplevel())
        while True:
            resp = dlg.run()
