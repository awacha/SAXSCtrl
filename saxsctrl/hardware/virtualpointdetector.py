...
class VirtualPointDetector(objwithgui.ObjWithGUI):
    name = GObject.property(
        type=str, default='--please set name--', blurb='Detector name')
    scaler = GObject.property(type=float, default=1, blurb='Scaling factor')
    visible = GObject.property(
        type=bool, default=True, blurb='Visible on graphs')
    ...
    def savestate(self, cp):
        objwithgui.ObjWithGUI.savestate(self, cp)
        cp.set(
            self._get_classname(), 'VPD_Type', self.__class__.__name__)
...
class VirtualPointDetectorExposure(VirtualPointDetector):
    mode = GObject.property(
        type=str, default='max', blurb='Image manipulation mode')
    ...
    def __init__(self, name, scaler=None, mask=None, mode=None):
        ...
        self._OWG_hints['mode'] = {objwithgui.OWG_Hint_Type.ChoicesList: [
            'max', 'min', 'mean', 'sum', 'barycenter_x', 'barycenter_y', 'sigma_x', 'sigma_y', 'sigma_tot']}
    ...
    def do_notify(self, prop):
        if prop.name == 'mask':
            try:
                ...
                logger.error(
                    'Cannot load mask: ' + self.mask + '. Reason: ' + str(ioe))
                self._mask = None
...
class VirtualPointDetectorGenix(VirtualPointDetector):
    genixparam = GObject.property(
        type=str, default='HT', blurb='GeniX parameter to record')

    def __init__(self, name, scaler=None, genixparam=None):
        self._OWG_init_lists()
        self._OWG_entrytypes[
            'genixparam'] = objwithgui.OWG_Param_Type.ListOfStrings
        self._OWG_hints['genixparam'] = {
            objwithgui.OWG_Hint_Type.ChoicesList: ['HT', 'Status', 'Current', 'TubeTime']}
        VirtualPointDetector.__init__(self, name, scaler)
        if genixparam is not None:
...
class VirtualPointDetectorHeader(VirtualPointDetector):
    headerfield = GObject.property(
        type=str, default='FSN', blurb='Header field')

    def __init__(self, name, scaler=None, headerfield=None):
        self._OWG_init_lists()
        self._OWG_entrytypes[
            'headerfield'] = objwithgui.OWG_Param_Type.ListOfStrings
...
def virtualpointdetector_new_from_configparser(name, cp):
    if not cp.has_section('VPD_' + name):
        raise ValueError(
            'No definition for detector "' + name + '" in config file.')
    ...
    try:
        cls = [c for c in VirtualPointDetector.__subclasses__() if c.__name__ == cp.get(
            'VPD_' + name, 'VPD_Type')][0]
    except IndexError:
        raise ValueError('Detector type %s not implemented!' %
                         (cp.get('VPD_' + name, 'VPD_Type')))
    det = cls(name, scaler)
    det.loadstate(cp)
    return det
