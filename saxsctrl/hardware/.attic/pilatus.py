...
class PilatusCommand(object):
    ...
    def __init__(self, cmd, context, regexes_dicts=None, argtypes=None):
        ...
        if regexes_dicts is None:
            ...
            if argtypes is None:
                raise ValueError(
                    'argument argtypes cannot be None if regexes is not None.')
    ...
    def parse(self, context, status, mesg):
        ...
        if self.regexes is None:
            ...
            logger.debug(
                'Matched command ' + self.cmd + ' based on context number alone.')
            yield gd
            return
        for regex in self.regexes:
            for m in regex.finditer(mesg):
                ...
                logger.debug(
                    'Matched command ' + self.cmd + ' based on regexp.')
                yield gd
        return
...
PilatusCommand('ExpTime', 15, [
               ('Exposure time set to: (?P<exptime>\d+\.\d+) sec.', {})], {'exptime': float})
...
PilatusCommand(
    'ImgMode', 15, [('ImgMode is (?P<imgmode>.*)', {})], {'imgmode': str})
...
PilatusCommand('ExpPeriod', 15, [('Exposure period set to: (?P<expperiod>\d+.\d+) sec', {})],
               {'expperiod': float})
PilatusCommand(
    'NImages', 15, [('N images set to: (?P<nimages>\d+)', {})], {'nimages': float})
PilatusCommand('SetAckInt', 15, [
               ('Acknowledgement interval is (?P<ackint>\d+)', {})], {'ackint': int})
...
class PilatusCommProcess(multiprocessing.Process):
    ...
    def handle_event(self, contextnumber, status, mesg, epoch):
        logger.debug('Handling event in comm process. Context: ' +
                     str(contextnumber) + '; Status: ' + status + '; Message: ' + mesg)
        dic = None
        for c in self._commands:
    ...
    def disconnect_from_camserver(self):
        if self.socket is None:
            raise PilatusError(
                'Cannot disconnect: connection is already closed.')
        self.killswitch.set()
        self.join()
    ...
    def _process_sending(self):
        while True:
            ...
            # from there. This is needed, since multiprocessing.Queue does not
        ...
        while msgtosend is not None:
            ...
            with self.pilatus_lock:
                writable, exceptional = select.select(
                    [], [self.socket], [self.socket], self.iotimeout)[1:3]
            if exceptional:
                raise CommunicationError('Exceptional state on send')
            if not writable:
                # put the message back to the priority queue, with an increased
                # priority
                self.sendqueue2.put(Message(msgtosend))
                return
            # if we have a writable socket to send our message to, separate the
            # command part and check if we have a corresponding event.
            cmd = str(msgtosend).strip().split()[0]
            try:
                # add a whitespace to the end. This is needed because of a bug
                # in camserver
                mesg1 = str(msgtosend)
                # pad the message to the last used message length to work
                # around a bug in camserver, which does not erase the last
                # input string buffer.
    ...
    def _process_receiving(self):
        mesg = []
        with self.pilatus_lock:
            readable, exceptional = select.select(
                [self.socket], [self.socket], [self.socket], self.iotimeout)[0:3:2]
            if exceptional:
                raise CommunicationError('Exceptional state on read')
            while readable:
                newmesg = self.socket.recv(self.mesglen)
                # was readable but 0 bytes received: socket closed.
                if not newmesg:
                    raise CommunicationError('Socket closed by camserver')
                mesg.append(newmesg)
                readable, exceptional = select.select(
                    [self.socket], [self.socket], [self.socket], 0)[0:3:2]
                if exceptional:
                    raise CommunicationError('Exceptional state on read')
        if mesg:
            mesg = ''.join(mesg)
        ...
        for msg in [m for m in mesg.split('\x18') if m]:
            try:
                ...
                data = re.match(
                    '^(?P<context>\d+)\s+(?P<state>\w+)\s*(?P<message>.*)', msg, re.DOTALL | re.MULTILINE).groupdict()
                contextnumber = int(data['context'])
                state = data['state']
                message = data['message']
            # raised if the regular exception does not match
    ...
    def run(self):
        ...
        # this function constitutes of a main loop, which can be ended via a
...
class PilatusConnection(GObject.GObject):
    ...
    def poll_commqueue(self, blocking=False, timeout=0.3):
        if not self.connected():
            raise PilatusError('Not connected to camserver.')
        handled = []
        while True:
            try:
                cmd, status, dic, epoch, message = self.commprocess.outqueue.get(
                    blocking, timeout)
            except multiprocessing.queues.Empty:
                break
            # this can happen if self.commprocess becomes None. In this case we
            # should also exit.
            except AttributeError:
            ...
            if self.lastresults[cmd][0] != epoch:
                ...
                self.lastresults[cmd] = (
                    epoch, status, add_dicts(self.lastresults[cmd][2], dic))
            handled.append(cmd)
            self.emit(
                'camserver-' + cmd, status, *[dic[k] for k in sorted(dic.keys())])
            if status == 'ERR':
    ...
    def connect_to_camserver(self):
        ...
        def _handler():
            ...
            # if self.commprocess became None, i.e. the communications have
            # been closed down.
            except AttributeError:
    ...
    def expose(self, exptime, filename, Nimages=1, expdwelltime=None):
        if expdwelltime is None:
            expdwelltime = 0.003
        logger.info('Starting the exposure of ' + str(Nimages) + ' images, each for ' + str(
            exptime) + ' secs. Filename: ' + str(filename) + '; dwell time ' + str(expdwelltime))
        ...
        if Nimages == 1:
            logger.debug(
                'Starting %f seconds exposure to filename %s.' % (exptime, filename))
        else:
            logger.debug('Starting %d exposures. Exptime: %f seconds. Period: %f seconds. Starting file: %s' % (
                Nimages, exptime, exptime + expdwelltime, filename))
        return self.lastresults['Exposure'][2]
    ...
    class reset_and_wait_for_event(object):
        ...
        def __enter__(self):
            logger.debug(
                'Resetting last results for command(s): ' + str(self.commandnames))
        ...
        def __exit__(self, exc_type, exc_value, traceback):
            if exc_type is not None:
                logger.error(
                    'An exception occurred before waiting for results: ' + str(exc_value))
                return False
            logger.debug(
                'Starting waiting loop for command(s): ' + str(self.commandnames))
            # empty the queue if there are pending events
            self.pconnection.poll_commqueue()
            time0 = time.time()
    ...
    def stopexposure(self):
        ...
        self.send('k')
        # we do not wait_for_event() here, because another thread may be doing
        # the same.
        return
    ...
    def setthreshold(self, threshold, gain, blocking=True):
        logger.debug('Setting threshold to %d eV (gain: %s)' %
                     (threshold, gain))
        if blocking:
