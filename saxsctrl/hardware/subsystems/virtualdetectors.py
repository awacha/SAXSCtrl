...
class AddDetectorDialog(Gtk.Dialog):
    ...
    def __init__(self, credo, title='Add detector', parent=None):
        ...
        l = Gtk.Label(label='Detector type:')
        ...
        for t in VirtualPointDetector.__subclasses__():
            self.type_entry.append_text(
                t.__name__.replace('VirtualPointDetector', ''))
        self.type_entry.set_active(0)
        self.type_entry.connect(
            'changed', lambda combo: self.on_type_entry_change())
    ...
    def on_type_entry_change(self):
        if self._table is not None:
            ...
            del self._table
            del self._detector
        cls = [c for c in VirtualPointDetector.__subclasses__() if c.__name__.replace(
            'VirtualPointDetector', '') == self.type_entry.get_active_text()][0]
        self._detector = cls()
...
class DetectorTypeSelector(Gtk.Dialog):
    ...
    def __init__(self, parent):
        Gtk.Dialog.__init__(self, 'Select detector type to add',
                            transient_for=parent, destroy_with_parent=True,
                            modal=True, buttons=(Gtk.STOCK_OK,
                                                 Gtk.ResponseType.OK,
                                                 Gtk.STOCK_CANCEL,
        ...
        for c in VirtualPointDetector.__subclasses__():
            self._typeselector.append_text(
                c.__name__.replace('VirtualPointDetector', ''))
        self._typeselector.set_active(0)

    def get_detectorclass(self):
        return [c for c in VirtualPointDetector.__subclasses__() if
                c.__name__.replace('VirtualPointDetector', '') ==
                self._typeselector.get_active_text()][0]


class DetectorListTable(Gtk.Frame):
    __gtype_name__ = 'SAXSCtrl_DetectorListTable'
    ...
    def __init__(self, subsys):
        ...
        self._detview.append_column(
            Gtk.TreeViewColumn('Name', Gtk.CellRendererText(), text=1))
        self._detview.append_column(
            Gtk.TreeViewColumn('Type', Gtk.CellRendererText(), text=2))
        self._detview.append_column(
            Gtk.TreeViewColumn('Scaling', Gtk.CellRendererText(), text=3))
        self._detview.append_column(
            Gtk.TreeViewColumn('Visibility', Gtk.CellRendererToggle(), active=4))
        self._detview.append_column(
            Gtk.TreeViewColumn('Arguments', Gtk.CellRendererText(), text=5))
    ...
    def _update_model(self):
        for row in self._detlist:
            row[1] = row[0].name
            row[2] = row[0].__class__.__name__.replace(
                'VirtualPointDetector', '')
            row[3] = row[0].scaler
            row[4] = row[0].visible
            row[5] = str(row[0])
    ...
    def _add(self):
        ...
        if dia.run() == Gtk.ResponseType.OK:
            self._detlist.prepend(
                [dia.get_detectorclass()('--please name this detector--'), '', '', 0.0, False, ''])
            self._update_model()
        dia.destroy()
        del dia
        self._detview.get_selection().select_iter(
            self._detlist.get_iter_first())
        self._edit()
    ...
    def _edit(self):
        model, it = self._detview.get_selection().get_selected()
        if it is not None:
            dia = model[it][0].create_setup_dialog(parent=self.get_toplevel(
            ), flags=Gtk.DialogFlags.DESTROY_WITH_PARENT | Gtk.DialogFlags.MODAL)
    ...
    def do_response(self, respid):
        if respid == 10:
            ...
            md = Gtk.MessageDialog(self, Gtk.DialogFlags.DESTROY_WITH_PARENT | Gtk.DialogFlags.MODAL,
                                   Gtk.MessageType.INFO, Gtk.ButtonsType.OK, 'Detectors saved to file ' + self._tab.objwithgui.configfile)
            ...
            md = Gtk.MessageDialog(self, Gtk.DialogFlags.DESTROY_WITH_PARENT | Gtk.DialogFlags.MODAL,
                                   Gtk.MessageType.INFO, Gtk.ButtonsType.OK, 'Detectors loaded from file ' + self._tab.objwithgui.configfile)
            md.set_title('Information')
            md.run()
            md.destroy()
            del md
...
class SubSystemVirtualDetectors(SubSystem):
    ...
    def save(self, filename=None):
        if self.offline:
            logger.warning(
                'Not saving virtual detector table: we are in offline mode.')
            return
        if filename is None:
            filename = self.configfile
        cp = configparser.ConfigParser()
