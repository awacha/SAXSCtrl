...
class SubSystemMotors(SubSystem):
    ...
    def __init__(self, credo, offline=True):
        ...
        for md in [self.credo().get_equipment(md) for md in self.credo().subsystems['Equipments'].get_motor_drivers()]:
            for signame in ['motors-changed', 'motor-start', 'motor-report', 'motor-stop', 'motor-limit', 'motor-settings-changed']:
                self._conns.append((weakref.ref(md), md.connect(signame, self._proxy_signal, signame)))
                self._driverconns.append((md, md.connect('connect-equipment', self._on_driver_connect)))
                self._driverconns.append((md, md.connect('disconnect-equipment', self._on_driver_disconnect)))
                self._driverconns.append((md, md.connect('idle', self._on_driver_idle)))
    def __del__(self):
    ...
    def get_motors(self):
        ...
        for md in [dr for dr in self.credo().subsystems['Equipments'].get_motor_drivers() if self.credo().subsystems['Equipments'].is_connected(dr)]:
            mots.extend(self.credo().subsystems['Equipments'].get(md).motors.values())
    ...
    def get(self, motorname, casesensitive=True):
        ...
        lis = [m for m in mots if m.name == motorname or ((not casesensitive) and m.name.upper() == motorname.upper())]
        if len(lis) == 1: return lis[0]
        lis = [m for m in mots if m.alias == motorname or ((not casesensitive) and m.alias.upper() == motorname.upper())]
        if len(lis) == 1: return lis[0]
        lis = [m for m in mots if str(m) == motorname or ((not casesensitive) and str(m).upper() == motorname.upper())]
        if len(lis) == 1: return lis[0]
