    ...
    def new_from_name(cls, credo, devicename, **args):
        ...
        for c in cls.__subclasses__():
            if c.match_name(name):
                return c(credo, arg)
        raise NotImplementedError(
            'No descendant of class ScanDevice matches device %s' % str(devicename))
...
class ScanDeviceMotor(ScanDevice):
    ...
    def validate_interval(self, start, end, nsteps, ct, wt):
        softlimits = self.motor.get_softlimits_phys()
        left = min(softlimits)
        right = max(softlimits)
        if not ((start >= left) and (start <= right)):
            logger.warning(
                'Scan interval validation failed: starting point not inside software limits.')
            return False
        elif not ((end >= left) and (end <= right)):
            logger.warning(
                'Scan interval validation failed: end point not inside software limits.')
            return False
        elif nsteps < 2:
            logger.warning(
                'Scan interval validation failed: not enough steps.')
            return False
        else:
            return True
...
class ScanDevicePilatus(ScanDevice):
    ...
    def __init__(self, credo, what):
        ...
        if what.lower() != 'threshold':
            raise NotImplementedError(
                'Only "threshold" scans are supported for Pilatus scandevices.')
        self.arg = what
...
class SubSystemScan(SubSystem):
    ...
    nstep = GObject.property(
        type=int, minimum=2, default=2, blurb='Number of steps (>=2)')
    waittime = GObject.property(
        type=float, minimum=0, default=0, blurb='Wait time (sec)')
    devicename = GObject.property(
        type=str, default='Time:Clock', blurb='Device name')
    ...
    def _init_scan_device(self):
        self.scandevice = ScanDevice.new_from_name(
            self.credo(), self.devicename)
    ...
    def prepare(self, header_template={}, mask=None):
        ...
        self.currentscan.motorpos = [m.get_parameter('Current_position')
                                     for m in self.credo().subsystems['Motors'].get_motors()]
        step = (self.value_end - self.value_begin) / (self.nstep - 1)
        command = 'scan ' + str(self.scandevice) + ' from ' + str(self.value_begin) + ' to ' + str(
            self.value_end) + ' by ' + str(step) + ' ct = ' + str(self.countingtime) + ' wt = ' + str(self.waittime)
        ...
        self.currentscan.start_record_mode(
            command, self.nstep, self.credo().subsystems['Files'].scanfile)
        ...
        if self.scandevice.name() == 'Time':
            ...
            self.credo().subsystems['Exposure'].dwelltime = self.value_end / (
                self.nstep - 1) - self.countingtime
        else:
            self.credo().subsystems['Exposure'].nimages = 1
        self.credo().subsystems['Exposure'].timecriticalmode = True
        self.credo().subsystems['Files'].filebegin = 'scn'
    ...
    def _exposure_image(self, sse, ex):
        ...
        if self.scandevice.name() == 'Time':
            if self._firsttime is None:
                ...
                where = float(
                    ex['CBF_Date'].strftime('%s.%f')) - self._firsttime
        else:
            where = self._where
    ...
    def _do_next_step(self):
        ...
        try:
            logger.debug('Scan step %d/%d' %
                         (self._current_step + 1, self.nstep))
    ...
    def _start_exposure(self):
        ...
        logger.debug(
            'Done starting exposure in scan sequence. FSN will be: %d' % fsnwillbe)
        return False
