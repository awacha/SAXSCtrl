...
class SubSystemImaging(SubSystem):
    ...
    nstep1 = GObject.property(type=int, minimum=2, default=2, blurb='Number of steps #1 (>=2)')
    nstep2 = GObject.property(type=int, minimum=2, default=2, blurb='Number of steps #2 (>=2)')
    waittime = GObject.property(type=float, minimum=0, default=0, blurb='Wait time (sec)')
    devicename1 = GObject.property(type=str, default='Time:Clock', blurb='Device name #1')
    devicename2 = GObject.property(type=str, default='Time:Clock', blurb='Device name #1')
    ...
    def _init_scan_devices(self):
        self.scandevice1 = ScanDevice.new_from_name(self.credo(), self.devicename1)
        self.scandevice2 = ScanDevice.new_from_name(self.credo(), self.devicename2)
    ...
    def prepare(self, header_template={}, mask=None):
        ...
        command = 'imaging (' + str(self.scandevice1) + ', ' + str(self.scandevice2) + ') from (' + str(self.value_begin1) + ', ' + str(self.value_begin2) + \
                ') to (' + str(self.value_end1) + ', ' + str(self.value_end2) + ') by (' + str(step1) + ', ' + str(step2) + ') ct = ' + str(self.countingtime) + ' wt = ' + str(self.waittime)
        ...
        self.currentscan.start_record_mode(command, (self.nstep1, self.nstep2), self.credo().subsystems['Files'].scanfile)
    ...
    def _exposure_image(self, sse, ex):
        ...
        vdreadout.update({self.scandevice1.name():where[0], self.scandevice2.name():where[1], 'FSN':ex['FSN']})
