...
#             ss.savestate(cp)
    ...
    def expose(self, exptime, nimages=1, dwelltime=0.003, mask=None, header_template=None, filebegin=None, write_nexus=True):
        ...
        return self.subsystems['Exposure'].start(header_template, mask, write_nexus=write_nexus)
    ...
    def update_nexustree(self, root, data, header):
        ...
        root.entry.duration = (dateutil.parser.parse(root.entry.end_time) - dateutil.parser.parse(root.entry.start_time)).total_seconds()
    ...
    def prepare_nexustree(self, mask, number, sample_or_title, exposuretype='single'):
        ...
        if exposuretype == 'scan':
            ...
            root.entry.definition.attrs['URL'] = 'http://svn.nexusformat.org/definitions/trunk/applications/NXscan.nxdl.xml'
            ...
            root.entry.title = self.subsystems['Files'].filebegin + '_%d' % number
        ...
        root.entry.program_name.attrs['version'] = pkg_resources.get_distribution('saxsctrl').version
        ...
        if exposuretype == 'scan':
            root.entry.control.data = nxs.NXfield(dtype='float32', shape=(nxs.UNLIMITED,))
            root.entry.control.count_time = nxs.NXfield(dtype='float32', shape=(nxs.UNLIMITED,))
        elif exposuretype == 'imaging':
            ssi = self.subsystems['Imaging']
            root.entry.control.data = nxs.NXfield(dtype='float32', shape=(ssi.nstep1, ssi.nstep2))
            root.entry.control.count_time = nxs.NXfield(dtype='float32', shape=(ssi.nstep1, ssi.nstep2))
        elif exposuretype == 'single':
        ...
        root.entry.instrument.monochromator.energy = scipy.constants.codata.value('Planck constant in eV s') * scipy.constants.codata.value('speed of light in vacuum') * 1e10 / self.wavelength
        ...
        if isinstance(sample_or_title, basestring):
            ...
            root.entry.sample.transmission = float(sample_or_title.transmission)
            ...
            root.entry.sample.geometry.translation.distances = np.array([sample_or_title.positionx,
        ...
        det.frame_time = self.subsystems['Exposure'].exptime + self.subsystems['Exposure'].dwelltime
        ...
        if exposuretype == 'scan':
            det.data = nxs.NXfield(dtype='int32', shape=(nxs.UNLIMITED, pilatus.hpix, pilatus.wpix))
            sss = self.subsystems['Scan']
            det.data.attrs['axes'] = sss.scandevice.name() + ':x:y'
            root.entry.data.insert(sss.scandevice.name(), nxs.NXfield(dtype='float32', shape=(nxs.UNLIMITED,)))
        elif exposuretype == 'imaging':
            det.data = nxs.NXfield(dtype='int32', shape=(ssi.nstep1, ssi.nstep2, pilatus.hpix, pilatus.wpix))
            name1 = ssi.scandevice1.name()
            name2 = ssi.scandevice2.name()
            det.data.attrs['axes'] = name1 + ':' + name2 + ':x:y'
            root.entry.data.insert(name1, nxs.NXfield(dtype='float32', shape=(ssi.nstep1,)))
            root.entry.data.insert(name2, nxs.NXfield(dtype='float32', shape=(ssi.nstep2,)))
            root.entry.data[name1].attrs['long_name'] = ssi.devicename1
            root.entry.data[name2].attrs['long_name'] = ssi.devicename2
        else:
            det.data = nxs.NXfield(dtype='int32', shape=(pilatus.hpix, pilatus.wpix))
        ...
        for vd in self.subsystems['VirtualDetectors']:
            ...
            if self.exposuretype == 'scan':
                root.entry.instrument[vd.name].insert(vd.name, nxs.NXfield(dtype='float32', shape=nxs.UNLIMITED,))
