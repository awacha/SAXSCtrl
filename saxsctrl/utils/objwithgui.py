    ...
    def do_response(self, respid):
        ...
        if respid in (Gtk.ResponseType.CANCEL, Gtk.ResponseType.DELETE_EVENT, Gtk.ResponseType.REJECT):
            if respid == Gtk.ResponseType.DELETE_EVENT:
                logger.debug('Delete-event in ObjSetupDialog for %s' % self._tab.objwithgui._get_classname())
...
class ObjSetupTable(Gtk.Table):
    ...
    def __init__(self, objwithgui, rows=1, columns=1, homogeneous=False):
        ...
        for p in sorted(list(self.objwithgui.props), key=lambda p:self.objwithgui._get_priority(p.name)):
            ...
            if self.objwithgui._OWG_entrytypes[p.name] == OWG_Param_Type.Bool:
                ...
                self._entries[p.name]._connection = self._entries[p.name].connect('toggled', lambda cb, n: self.emit('changed', n), p.name)
                ...
                if self.objwithgui._OWG_entrytypes[p.name] == OWG_Param_Type.String:
                    ...
                    self._entries[p.name]._connection = self._entries[p.name].connect('value-changed', lambda sb, n: self.emit('changed', n), p.name)
                ...
                logger.debug('Initialized entry for property %s: %d' % (p.name, id(self._entries[p.name])))
        ...
        for i, owgp in enumerate(self.objwithgui._OWG_parts):
            ...
            tab.attach(b, i % self.objwithgui._OWG_parttab_cols, i % self.objwithgui._OWG_parttab_cols + 1,
                       i / self.objwithgui._OWG_parttab_cols, i / self.objwithgui._OWG_parttab_cols + 1)
            b._connection = b.connect('clicked', lambda b, part:self._create_owgpart_setupdialog(part), owgp)
            self._partbuttons.append(b)
            
            
        self._owgconn = self.objwithgui.connect('notify', self._objwithgui_notify)
    def do_destroy(self):
    ...
    def _create_owgpart_setupdialog(self, owgp):
        dia = owgp.create_setup_dialog(parent=self.get_toplevel(), flags=Gtk.DialogFlags.DESTROY_WITH_PARENT)
        while True:
            result = dia.run()
...
class ObjWithGUI(GObject.GObject):
    ...
    def _OWG_init_lists(self):
        ...
        if self._OWG_hints is None:
            self._OWG_hints = {}
        dic = {'__default__':{OWG_Hint_Type.Digits:4, OWG_Hint_Type.Editable:False, OWG_Hint_Type.ChoicesList:[]}}
    ...
    def loadstate(self, configparser, sectionprefix=''):
        ...
        for p in self.props:
            ...
            if not configparser.has_option(sectionprefix + self._get_classname(), p.name):
                continue
            if p.value_type.name == 'gboolean':
                val = configparser.getboolean(sectionprefix + self._get_classname(), p.name)
            elif p.value_type.name in ['gint', 'guint', 'glong', 'gulong',
                                       'gshort', 'gushort', 'gint8', 'guint8',
                                       'gint16', 'guint16', 'gint32', 'guint32',
                                       'gint64', 'guint64']:
                val = configparser.getint(sectionprefix + self._get_classname(), p.name)
            elif p.value_type.name in ['gfloat', 'gdouble']:
                val = configparser.getfloat(sectionprefix + self._get_classname(), p.name)
            else:
                val = configparser.get(sectionprefix + self._get_classname(), p.name)
    ...
    def savestate(self, configparser, sectionprefix=''):
        ...
        for p in self.props:
            if p.name in self._OWG_nosave_props:
                continue
            configparser.set(sectionprefix + self._get_classname(), p.name, self.get_property(p.name))
        for owgp in self._OWG_parts:
